...
export default function JobDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 py-8">
        <Link href="/jobs" className="inline-flex items-center gap-2 text-primary hover:text-blue-900 font-semibold mb-6 smooth-enter-left animation-delay-100 transition-all duration-300 hover:translate-x-1">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 smooth-enter animation-delay-200">
            <div className="card-base p-8 mb-6 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-500 border border-gray-200">
              <div className="flex items-start justify-between mb-6">
                ...
                <div className="flex gap-2 smooth-enter-right animation-delay-400">
                  <button type="button" aria-label="Save job" title="Save job" className="p-2 border border-gray-300 rounded-lg hover:bg-red-50 hover:border-red-300 transition-all duration-300 group">
                    <Heart className="w-6 h-6 text-gray-600 group-hover:text-red-500 group-hover:fill-red-500 transition-all duration-300" />
                  </button>
                  <button type="button" aria-label="Share job" title="Share job" className="p-2 border border-gray-300 rounded-lg hover:bg-blue-50 hover:border-blue-300 transition-all duration-300 group">
            ...
            <div className="card-base p-8 mb-6 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-500 border border-gray-200 fade-in animation-delay-800">
              ...
              <div className="prose prose-sm max-w-none text-gray-600 whitespace-pre-line hover:text-gray-700 transition-colors duration-300">{job.description}</div>
            </div>
            ...
            <div className="card-base p-8 mb-6 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-500 border border-gray-200 fade-in animation-delay-900">
              ...
              <div className="flex flex-wrap gap-3">
                {job.skills.map((skill, idx) => (
                  <div key={skill} className="bg-linear-to-r from-primary/10 to-primary/5 text-primary px-4 py-2 rounded-full font-semibold fade-in transition-all duration-300 hover:shadow-md hover:scale-105 hover:from-primary/20 hover:to-primary/15" style={{animationDelay: `${900 + idx * 100}ms`}}>{skill}</div>
                ))}
              </div>
            </div>
            ...
            <div className="card-base p-8 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-500 border border-gray-200 fade-in animation-delay-1000">
              ...
              <Link href={`/companies/1`} className="text-primary hover:text-blue-900 font-semibold inline-flex items-center gap-2 transition-all duration-300 hover:translate-x-1">View Company Profile →</Link>
            </div>
          </div>

          <div className="smooth-enter-right animation-delay-300">
            <button type="button" className="w-full btn-primary mb-6 bg-linear-to-r from-primary to-primary/80 text-gray-900 font-bold py-3 rounded-lg transition-all duration-500 shadow-lg hover:shadow-xl hover:scale-105 active:scale-95">Apply for This Job</button>
            ...
            <div className="card-base p-6 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-500 border border-gray-200 fade-in animation-delay-600">
              ...
              <div className="space-y-4">
                {similarJobs.map((similarJob, idx) => (
                  <Link key={similarJob.id} href={`/jobs/${similarJob.id}`} className="block p-3 border border-gray-200 rounded-lg hover:border-primary hover:bg-blue-50 transition-all duration-300 group fade-in" style={{animationDelay: `${600 + idx * 100}ms`}}>
                    <h4 className="font-semibold text-gray-900 text-sm mb-1 group-hover:text-primary transition-colors duration-300">{similarJob.title}</h4>
                    <p className="text-xs text-gray-600 mb-2">{similarJob.company}</p>
                    <p className="text-xs text-gray-500 group-hover:text-primary/70 transition-colors duration-300">{similarJob.salary}</p>
                  </Link>
                ))}
              </div>
            </div>
