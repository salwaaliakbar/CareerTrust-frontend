...
const NotificationSidebar: React.FC<NotificationSidebarProps> = ({ notifications, onClose, onMarkAllRead }) => {
  return (
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {notifications.length === 0 ? (
          ...
          notifications.map((n) => (
            <div
              key={n.id}
              className={`rounded-lg p-4 border ${n.read ? 'bg-gray-50 border-gray-200' : 'bg-blue-50 border-blue-200'} shadow-sm flex flex-col gap-1`}
            >
              <div className="flex items-center gap-2">
                ...
                <span className="font-semibold text-blue-900 text-base">{n.title}</span>
              </div>
