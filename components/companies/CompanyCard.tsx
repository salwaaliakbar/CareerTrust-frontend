...
export default function CompanyCard({ company }: { company: Company }) {
  return (
    ...
    >
      ...
      <div
        className="absolute left-0 top-0 bottom-0 w-1 bg-linear-to-b from-gray-200 via-primary/40 to-gray-200 group-hover:bg-linear-to-b group-hover:from-primary group-hover:via-primary/60 group-hover:to-primary transition-all duration-500 rounded-full"
        aria-hidden="true"
      />

      <div className="relative z-10">
        {/* Logo - animated */}
        <div className="w-20 h-20 bg-linear-to-br from-[#0C2B4E] to-[#1D546C] rounded-full flex items-center justify-center mb-4 shadow-lg ring-4 ring-white transition-all duration-500 group-hover:scale-110 group-hover:shadow-xl group-hover:ring-blue-100 overflow-hidden">
          {company.logo &&
          (company.logo.startsWith("http") || company.logo.startsWith("/")) ? (
            ...
            <span className="text-white font-bold text-lg transition-transform duration-300 group-hover:rotate-12">
              {company.name
                .split(" ")
                .slice(0, 2)
                .map((w) => w[0])
        ...
        <p className="text-sm text-gray-500 mb-3">
          <span className="inline-block bg-linear-to-r from-gray-100 to-gray-50 text-gray-700 px-2 py-1 rounded-full text-xs font-semibold group-hover:from-primary/10 group-hover:to-primary/5 group-hover:text-primary transition-all duration-300">
            {company.industry}
          </span>
        </p>
        ...
        <div className="space-y-3 mb-6 border-t border-gray-200 pt-4">
          <div className="flex items-center gap-2 text-sm transition-all duration-300 group-hover:text-primary">
            ...
            <span className="text-gray-500 group-hover:text-primary/70">
      ...
      <button
        className="w-full bg-linear-to-r from-[#0C2B4E] to-[#1D546C] text-white py-3 rounded-lg font-semibold transition-all duration-500 shadow-md hover:shadow-lg hover:scale-105 active:scale-95 group-hover:from-primary group-hover:to-primary/80"
        type="button"
        aria-label={`View ${company.name}`}
      >
        View Company
