          ...
          <div className="w-full md:w-1/2 flex items-center justify-center px-4">
            <div className="w-full max-w-4xl bg-[#F4F4F4] border border-gray-300 rounded-xl shadow-lg p-8 max-h-[calc(100vh-6rem)] overflow-auto">
              ...
              <form onSubmit={rhfHandleSubmit(onSubmit)} className="space-y-6 mb-8">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input id="email" type="email" {...register('email', { required: 'Email is required', pattern: { value: /^[^\s@]+@[^\s@]+\.[^\s@]+$/, message: 'Invalid email address' } })} placeholder="you@example.com" className={`w-full pl-10 pr-4 py-2.5 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#0C2B4E] focus:border-transparent ${rhfErrors.email ? 'border-red-400' : 'border-gray-300'}`} />
                    </div>
                    ...
                    <>
                      <div>
                        ...
                        <div className="relative">
                          ...
                          <input id="phone" {...register('phone', { required: 'Phone number is required', pattern: { value: /^[0-9]{10,15}$/, message: 'Invalid phone number (10-15 digits)' } })} placeholder="03xxxxxxxxx" className={`w-full pl-10 pr-4 py-2.5 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#0C2B4E] focus:border-transparent ${rhfErrors.phone ? 'border-red-400' : 'border-gray-300'}`} />
                        </div>
                      ...
                      <div>
                        ...
                        <div className="relative">
                          <User className="absolute left-3 top-3.5 w-5 h-5 text-gray-400" />
                          <input id="cnic" {...register('cnic', { required: 'CNIC is required', pattern: { value: /^[0-9]{13}$/, message: 'Invalid CNIC (must be 13 digits)' } })} placeholder="3510123456789" className={`w-full pl-10 pr-4 py-2.5 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#0C2B4E] focus:border-transparent ${rhfErrors.cnic ? 'border-red-400' : 'border-gray-300'}`} />
                        </div>
                  ...
                  {state.role === 'employer' && (
                    <>
                      ...
                      <div>
                        ...
                        <div className="relative">
                          <Building2 className="absolute left-3 top-3.5 w-5 h-5 text-gray-400" />
                          <input id="companyURL" {...register('companyURL', { required: state.role === 'employer' ? 'Company URL is required' : false, pattern: { value: /^https?:\/\//, message: 'Invalid URL (must start with http:// or https://)' } })} placeholder="https://company.com" className={`w-full pl-10 pr-4 py-2.5 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#0C2B4E] focus:border-transparent ${rhfErrors.companyURL ? 'border-red-400' : 'border-gray-300'}`} />
                        </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <Lock className="absolute left-3 top-3.5 w-5 h-5 text-gray-400" />
                      <input id="confirmPassword" type="password" {...register('confirmPassword', { required: 'Please confirm your password', validate: (v: string) => v === (document.getElementById('password') as HTMLInputElement | null)?.value || 'Passwords do not match' })} placeholder="••••••••" className={`w-full pl-10 pr-4 py-2.5 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#0C2B4E] focus:border-transparent ${rhfErrors.confirmPassword ? 'border-red-400' : 'border-gray-300'}`} />
                    </div>
                ...
                <div className="border-t pt-6 space-y-4">
                  <div className="flex items-start gap-3">
                    <input type="checkbox" id="terms" {...register('terms', { required: 'You must accept the terms and conditions' })} className="w-5 h-5 rounded border-gray-300 accent-[#0C2B4E] focus:ring-[#0C2B4E] cursor-pointer mt-1" />
                  ...
                  <button type="submit" disabled={state.isProcessing} className={`w-full inline-flex items-center justify-center gap-2 px-4 py-3 rounded-lg font-semibold text-lg transition ${state.isProcessing ? 'bg-gray-300 text-gray-700 cursor-not-allowed' : 'bg-[#0C2B4E] text-white hover:bg-[#1A3D64]'}`}>
                    {state.isProcessing ? (
                      <>
                        <svg className="animate-spin w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v4a4 4 0 00-4 4H4z"/></svg>
                        Processing...
                      </>
                    ) : (
                      <>
              ...
              <div className="text-center">
                <button onClick={() => dispatch({ type: "setStep", payload: 1 })} className="text-[#0C2B4E] hover:text-[#1A3D64] font-semibold">← Change role</button>
              </div>
            </div>
          </div>
        </main>
